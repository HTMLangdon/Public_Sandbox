...
export class PageComponent extends ComponentModule {
	...
	BindReport() {
		//debugger;
		let mod = this;
		var rptTitle = "Identity Management Report"; //@@@ Pending work, Replace with results from a service call
		var rptInfo = "HeaderInfo"; //@@@ Pending work,Replace with results from a service call

		mod.Services.ReportSvc.GetIdentityManagementReportData(mod.State.model.Params)
			.then(data => {
				if (data.ResponseCode == mod.Services.Constants.ResponseCode.Success) {
					if (data.Data.ReportData != null) {
						...
						var element = $("#reportResults").kendoGrid({
							toolbar: ["excel"],
							excel: {
								fileName: "IdentityManagementReport.xlsx",
								filterable: true,
								allPages: true
							},
							dataSource: {
