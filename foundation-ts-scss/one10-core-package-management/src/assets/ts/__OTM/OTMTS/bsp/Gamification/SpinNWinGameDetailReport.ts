...
export class PageComponent extends ComponentModule {
  ...
  Initialize() {
    ...
    model.set("Params.RptID", "GameDtlReport");
  ...
  BindReport() {
    let mod = this;
	  var rptTitle = "GameDtlReport"; //@@@ Pending work, Replace with results from a service call
    var rptInfo = "HeaderInfo"; //@@@ Pending work,Replace with results from a service call
   // debugger;
    mod.Services.ReportSvc.GetSpinnWinGameDetailReportData(mod.State.model.Params)
		.then(data => {
			debugger;
